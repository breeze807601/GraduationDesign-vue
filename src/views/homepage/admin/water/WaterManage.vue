<template>
    <div>
            ...
            <el-col :span="24">
                <el-form :model="form" inline>
                    ...
                    <el-form-item>
                        ...
                        <el-tooltip content="导出抄水表" placement="bottom" effect="light">
                            <el-button type="success" :icon="Download" auto-insert-space plain @click="download">导出</el-button>
                        </el-tooltip>
                        <el-tooltip content="导入抄水表" placement="bottom" effect="light">
        ...
        <div style="display: flex; align-items: flex-start;">
            <tariff :isAdmin="true" :isWater="true"></tariff>
        </div>
        ...
        <el-dialog v-model="dialogVisible" width="500" title="导入抄水表表格" :before-close="handleClose" destroy-on-close center>
            <!-- is-water判断是否是水表记录上传 -->
            <my-upload @close="close" :is-water="true"></my-upload>
        </el-dialog>
    </div>
</template>
...
async function getMeterList() {
    loading.value = true;
    await request.get('/waterMeter/list',{params: form.value}).then(res => {
...
async function download() {
    await request.get('/waterMeter/export', {
        ...
        link.href = url;
        link.setAttribute('download', '抄水表.xlsx'); // 设置下载文件名
...
async function handleConfirm(id) {
    await request.put('/waterMeter/update',null,{
        params:{
            id: id,
