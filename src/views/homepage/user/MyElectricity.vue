<template>
                    ...
                    <el-timeline-item v-for="item in billList" :timestamp="item.time" placement="top" hollow color="primary">
                        <el-card>
                            ...
                            <el-descriptions direction="vertical" :column="5" border >
                                <el-descriptions-item label="上次读数">{{ item.previousReading }} 度</el-descriptions-item>
                                <el-descriptions-item label="本次读数">{{ item.reading }} 度</el-descriptions-item>
                                <el-descriptions-item label="用电量">{{ item.summation }} 度</el-descriptions-item>
    ...
    <el-dialog v-model="dialog" title="详情" width="600">
        <el-descriptions direction="vertical" :column="4" border>
            ...
            <el-descriptions-item label="上次读数">{{ detailsInfo.previousReading }} 度</el-descriptions-item>
            <el-descriptions-item label="本次读数">{{ detailsInfo.reading }} 度</el-descriptions-item>
            <el-descriptions-item label="用电量">{{ detailsInfo.summation }} 度</el-descriptions-item>
            <el-descriptions-item label="总费用">{{ detailsInfo.cost }} 元</el-descriptions-item>
            <el-descriptions-item label="当时价格">{{ detailsInfo.price }} 元/度</el-descriptions-item>
            <el-descriptions-item label="可用额度">{{ detailsInfo.availableLimit }} 度</el-descriptions-item>
...
async function getBillList() {
    loading.value = true;
    await request.get('/electricityBill/getUserBill',{params: form.value}).then(res => {
...
async function settlement() {
    ...
    await request.post('/electricityBill/settlement',data).then(res => {
