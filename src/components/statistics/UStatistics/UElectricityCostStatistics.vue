<template>
    <el-card style="width: 42rem;" v-loading="loading">
        ...
        <div id="uElectricityCostOption" style="width: 40rem;height: 22rem;" />
    </el-card>
</template>
...
let uElectricityCostChart;
// 加载
const loading = ref(false);

const uElectricityCostOption = {
    title: {
        text: '用电费用情况'
    },
    tooltip: {
...
async function getCostStatistics() {
    loading.value = true;
    if (uElectricityCostChart != null && uElectricityCostChart !== "" && uElectricityCostChart !== undefined) {
        //销毁
        uElectricityCostChart.dispose();
    }
    uElectricityCostChart = echarts.init(document.getElementById('uElectricityCostOption'));
    await request.get('/electricityBill/getCostStatistics',{
        ...
        //横轴数据和纵抽数据
        uElectricityCostOption.xAxis.data = res.data.date;
        uElectricityCostOption.series[0].data = res.data.num;
    })
    loading.value = false;
    uElectricityCostChart.setOption(uElectricityCostOption);
}
